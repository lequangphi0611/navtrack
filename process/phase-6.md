...
- [ ] **A2 (thu hẹp 2026-07-21):** khi danh mục còn mã thiếu giá VÀ `missingPriceShare > 5%`, **treo** cảnh báo tập trung toàn danh mục; khi `≤ 5%`, vẫn cảnh báo bình thường trên các mã có giá kèm ghi chú NAV thiếu một phần — không báo động giả, không mất tín hiệu quá mức vì một mã nhỏ thiếu giá
- [ ] Danh mục có `100/n > threshold` (n = số Holding mở có giá): badge kèm ghi chú "tập trung tự nhiên do ít mã"
- [ ] Badge không nhấp nháy khi `concentrationPercent` dao động sát ngưỡng (hysteresis buffer 3 điểm %)
- [ ] Biểu đồ phân bổ tài sản có chú thích liên kết khi có Holding đang cảnh báo tập trung
