...
- 2026-07-11: **Regression ở trên không tái hiện được** — kiểm chứng lại bằng cookie session hợp lệ (fetch trực tiếp + `e2e/holdings.spec.ts`, `smoke.spec.ts`): cả 4 test pass, `/holdings` trả 200 đúng như session hợp lệ. Nghi vấn "Next.js 16 đổi middleware sang Edge runtime" **sai** — theo tài liệu chính thức (`node_modules/next/dist/docs/.../file-conventions/proxy.md`), từ v16.0.0 Proxy **mặc định dùng Node.js runtime** (ngược lại hoàn toàn so với nghi vấn ban đầu), nên Prisma gọi được bình thường. Khả năng cao đây là lỗi môi trường/cookie cục bộ nhất thời ở lần kiểm tra trước, không phải bug thật trong code. Không sửa gì thêm ở `proxy.ts`/`auth.ts`.
