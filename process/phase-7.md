...
- [x] Server Action ghi `Cashflow{type: MATURITY}`: prefill `date = maturityDate`, `quantity` = toàn bộ đang giữ, `pricePerUnit = parValue`, `taxAmount = max(0, (pricePerUnit − avgCost) × quantity) × thuế lãi theo issuerType`, `feeAmount` **mặc định `0`** (KHÔNG suy từ `TRANSACTION_FEE_SELL_BOND` — đảo lúc implement vì màn tất toán không có ô phí, xem `DECISION.md` 2026-07-29 (4)). Tất cả **chỉ prefill, không khoá**.
