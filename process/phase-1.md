...
- [ ] **Bảng `Setting`** (hạ tầng cấu hình) + enum `SettingValueType` + `resolveSetting(key, atDate)` + `prisma/seed.ts` (seed `MAX_MEMBERS`, `NAV_SNAPSHOT_*` mặc định; các key thuế seed sẵn để dùng ở Phase 4/5)
