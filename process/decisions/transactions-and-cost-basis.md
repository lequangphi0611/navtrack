...
- **Production chưa có record cổ tức cổ phiếu nào** (user xác nhận tại thời điểm này) → không cần script recompute/migration dữ liệu cũ, chỉ áp dụng cho giao dịch mới từ giờ trở đi. ⚠️ **Fact này đã LỖI THỜI** — production giờ đã có record cổ tức cổ phiếu thật; xem [`dividends.md`](./dividends.md) mục 2026-08-13 (bugfix `avgCost` không dilute qua cổ tức cổ phiếu + script backfill). Quyết định lõi của entry này ("một bộ đếm `realQuantity` duy nhất") vẫn đúng, không đổi.
...
- Bugfix `avgCost` PHẢI dilute qua cổ tức cổ phiếu (đảo quy tắc "avgCost không đổi bởi cổ tức" mà `derivePosition()`/`computeRealizedGainForHolding()` ở đây từng giữ) + backfill production — [`dividends.md`](./dividends.md), mục 2026-08-13.
