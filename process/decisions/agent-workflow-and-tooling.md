...
- Docs đã sync: `TOOLS.md` (dòng "Chạy E2E test"), `HARNESS.md` (mục "Verify khi hoàn thành"), `README.md` (mục mới "Chạy e2e trên Claude Cloud" — chứa nội dung script để copy + đoạn giải thích vì sao `pnpm install` không nằm trong đó), `docs/rules/testing.md` (2 chỗ: "nửa số phiên" + "DB riêng, ephemeral"), `e2e/CLAUDE.md` (mục "Cách chạy"), `.claude/agents/e2e-verifier.md` (bỏ auto-skip cứng trên Cloud, đổi theo tín hiệu `pg_isready` fail thật), `scripts/e2e.mjs`, `playwright.config.ts` (ép `executablePath` Chromium có sẵn trên Cloud — lỗi phát sinh độc lập với DB, do `@playwright/test` pin revision khác bản browser đã cài sẵn ở `/opt/pw-browsers`), `.claude/hooks/cloud-install-deps.sh` (mới), `.claude/settings.json` (thêm entry `hooks.SessionStart`).
- **Footgun phát hiện sau (cùng ngày, khi user dán script thật vào environment):** `apt-get update -qq` thoát exit 100 (`set -e` giết cả script) vì 2 PPA bên thứ 3 đã cấu hình sẵn trên environment (`deadsnakes`, `ondrej/php` — không liên quan gì tới Postgres) trả 403/"no longer signed" qua proxy của hạ tầng. Lúc spike trong session Claude Code (index apt đã có cache từ trước) cùng 2 PPA đó chỉ log **warning** (`W:`) chứ không chặn; trên environment thật (chưa có cache) cùng lỗi lại thành **error** (`E:`) khiến `apt-get update` trả exit khác 0. Vá bằng `apt-get update -qq || true` — chấp nhận archive Ubuntu chính vẫn fetch được dù PPA phụ lỗi, không cần lo whitelist/sửa nguồn PPA đó (ngoài phạm vi script này). User xác nhận script đã chạy được sau khi vá.
- **Bổ sung (cùng ngày, đọc kỹ tài liệu nền tảng theo yêu cầu của user "muốn cache install để vào phiên không cần cài lại"):** đọc [docs "Configure cloud environments"](https://code.claude.com/docs/en/cloud-environments) xác nhận nền tảng **đã tự cache kết quả setup script** — chạy đúng 1 lần cho environment, sau đó Anthropic snapshot filesystem và mọi phiên sau tái dùng snapshot đó (gói + role đã cài **không mất**), không chạy lại setup script trừ khi đổi setup script/network hoặc cache hết hạn (~7 ngày). Không cần user tự làm gì thêm để có "cache install" — nhưng phát hiện kèm theo **một bug thật trong code đã viết trước đó**: snapshot chỉ giữ THỨ GHI RA ĐĨA, không giữ **tiến trình đang chạy** — nghĩa là service Postgres luôn tắt ở đầu mọi phiên MỚI (kể cả phiên tái dùng snapshot với gói đã cài sẵn), nhưng `checkAndResetCloudDb()` bản trước coi "Postgres chưa `pg_isready`" là tín hiệu duy nhất = "setup script chưa cấu hình" rồi thoát lỗi — sai với đúng trường hợp bình thường nhất (phiên thứ 2 trở đi trong cùng environment). Sửa: tách 2 tín hiệu — `dpkg -s postgresql` (chưa cài → lỗi thật, báo setup script) vs `pg_isready` (đã cài nhưng service tắt → tự `service postgresql start`, không phải lỗi). Đã verify lại bằng cách giả lập đúng kịch bản (`service postgresql stop`, giữ nguyên gói đã cài) rồi chạy `pnpm e2e` — tự start service, 45/45 pass. Setup script trong README.md cũng đơn giản hoá: bỏ hẳn nhánh `service postgresql start` có điều kiện (không cần nữa vì service dù có start trong setup script cũng không sống sót qua cache) — chỉ giữ 1 lần start tạm thời để tạo role.
- **Từ chối đưa `pnpm install` vào setup script (yêu cầu tiếp theo của user), dùng SessionStart hook thay.** Lý do gốc rễ giống hệt điểm trên: setup script cache theo **environment** (~7 ngày), còn repo được **clone mới hoàn toàn mỗi phiên** — cache cả `node_modules` nghĩa là đóng băng dependency theo trạng thái repo của lần chạy setup script đầu tiên, các phiên sau (khác commit/lockfile) dùng nhầm bản cũ mà không có tín hiệu báo lỗi nào. Tài liệu nền tảng nói thẳng: setup script cho toolchain/OS package, **SessionStart hook** cho việc cài theo repo như `npm install` (chạy sau khi đã clone, luôn khớp bản đang có). Quyết định: thêm `.claude/hooks/cloud-install-deps.sh` (check `CLAUDE_CODE_REMOTE`, chỉ chạy trên Cloud) + đăng ký thêm entry trong `.claude/settings.json` mục `hooks.SessionStart` (matcher `startup|resume`, giữ nguyên hook `session-start.sh` cũ ở entry riêng — không gộp 2 việc khác nhau vào 1 script).
