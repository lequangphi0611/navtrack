...
- ⚠️ **Action còn treo — backfill production:** chạy `prisma/backfill/backfill_holding_position.sql` **1 lần** trên prod sau `migrate deploy` (recursive CTE bản sao `derivePosition`, gồm reset avgCost khi SL về 0; đã **verify khớp** trên local: 4 holding thật + 4 kịch bản synthetic). Migration `20260711081325_add_holding_position_cache` + backfill trên **local đã chạy**.
