...
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Code TypeScript/Next.js (app chính, `src/`, `prisma/`) | `pnpm lint && pnpm typecheck && pnpm test` — thêm `pnpm build` nếu đổi cấu hình Next/route; `pnpm e2e` (Playwright) chỉ khi đổi luồng UI xuyên trang, không chạy mặc định vì chậm                                                                                     |
| Schema Prisma (`prisma/schema.prisma`)                 | `pnpm prisma generate` rồi `pnpm prisma migrate dev` (tạo + áp migration local) trước khi `pnpm typecheck`/`pnpm test` — client Prisma phải đồng bộ schema mới build đúng                                                                                             |
| Job Python (`jobs/price-fetcher/`)                     | Trong `jobs/price-fetcher`: `pytest` (hoặc `python -m pytest`) rồi `ruff check .` và `ruff format --check .` — **không** tự chạy `python main.py` (ghi thật vào `PriceQuote`) trừ khi cần verify tích hợp có chủ đích và đã xác nhận `DATABASE_URL` trỏ đúng DB local |
