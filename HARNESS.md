...
| Job Python (`jobs/price-fetcher/`, `jobs/snapshot-cron/`) | Trong thư mục job: `pytest` (mặc định loại `integration`, nhanh) rồi `ruff check .` và `ruff format --check .`; nếu job đã có `test_integration.py` (hiện tại: `jobs/snapshot-cron/`), chạy thêm `pnpm test:python-integration` ở gốc repo trước khi báo xong — verify idempotent/giá trị trên DB thật. Không tự chạy `python main.py` (ghi thật) trừ khi cần verify tích hợp có chủ đích. |
