...
**Bảng trên viết cho Claude Local (hạ tầng tối thiểu bắt buộc).** Các bước cần Docker trực
tiếp (`pnpm test:python-integration`, `docker compose up`) không chạy được trên Claude Cloud
(không có Docker daemon) — xem [`TOOLS.md`](./TOOLS.md) để biết việc nào skip trên Claude
Cloud và việc nào vẫn bắt buộc. **`pnpm e2e` là ngoại lệ** — chạy được trên cả hai hạ tầng
(Cloud dùng Postgres native cài qua setup script của environment thay Docker — script cấu
hình ngoài repo, nội dung ở README.md mục "Chạy e2e trên Claude Cloud"); nếu setup script
chưa cấu hình cho environment đang chạy,
`pnpm e2e` tự báo lỗi rõ ("Postgres chưa sẵn sàng...") — lúc đó mới báo "chưa verify e2e được
ở đây" thay vì pass giả, không tự bịa cách chạy thay thế.
