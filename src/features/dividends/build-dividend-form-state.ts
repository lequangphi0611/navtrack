...
export function buildDividendFormState(
  ...
  const paymentDateFields = input.paymentDate
    ...
    : {};
  // Chỉ set khi CÓ điều chỉnh thật sự xảy ra (resolveOldPriceInTx không null
  // VÀ compute*PriceAdjustment không null) — vắng mặt = ẩn khối "Đã điều
  // chỉnh giá" (issue #61, xem comment DividendRecordedResult.types.ts).
  const priceAdjustmentFields = input.priceAdjustment
    ? {
        navOverrideAdjusted: true as const,
        oldPrice: input.priceAdjustment.oldPrice.toString(),
        newPrice: input.priceAdjustment.newPrice.toString(),
      }
    : {};
