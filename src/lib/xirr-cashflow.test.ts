...
describe("buildXirrCashflows — ghép dòng tiền giả định = NAV tại mốc chốt (docs/domain/05)", () => {
  ...
  test("vị thế mở + NAV=0 HỢP LỆ (mất trắng, có định giá thật, không phải thiếu giá) -> ghép điểm NAV isNavPoint:true, XIRR = -100%, KHÔNG phải NO_POSITIVE_FLOW", () => {
    const cutoffDate = new Date("2024-01-01");
    const points = buildXirrCashflows({
      cashflows: [
        { date: new Date("2023-01-01"), amount: new Decimal(-100_000_000) },
      ...
      dividends: [
        {
          ...
          paymentDate: null,
          netAmount: new Decimal(2_000_000),
        },
      ],
      isOpenPosition: true,
      cutoffDate,
  ...
  test("cổ tức có paymentDate trễ hơn date -> điểm dòng tiền XIRR nằm tại paymentDate, không phải date (issue #65)", () => {
    const cutoffDate = new Date("2024-01-01");
    const points = buildXirrCashflows({
      cashflows: [
        { date: new Date("2023-01-01"), amount: new Decimal(-100_000_000) },
      ...
      dividends: [
        {
          date: new Date("2023-06-01"), // ngày chia
          paymentDate: new Date("2023-06-20"), // tiền thực về, trễ ~3 tuần
          netAmount: new Decimal(2_000_000),
        },
      ],
      isOpenPosition: true,
  ...
  test("cổ tức không có paymentDate (null) -> fallback đúng về date (hành vi cũ không bị phá, issue #65)", () => {
    const cutoffDate = new Date("2024-01-01");
    const points = buildXirrCashflows({
      cashflows: [
        { date: new Date("2023-01-01"), amount: new Decimal(-100_000_000) },
