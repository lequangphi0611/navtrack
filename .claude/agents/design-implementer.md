...
8. Nếu bước 2 vừa fetch mockup mới (không phải đọc từ cache có sẵn): ghi nguyên văn ra `.claude/design-cache/raw/<tên-file-đã-sanitize>.html` và thêm/cập nhật entry tương ứng trong `.claude/design-cache/index.json`. Đọc từ cache có sẵn thì bỏ qua bước này, không cần ghi lại.
