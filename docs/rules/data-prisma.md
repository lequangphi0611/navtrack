...
- **Không tin `userId` từ client.** Luôn lấy từ session (Auth.js) phía server.
...
- **Không truyền thẳng model Prisma ra client.** Convert `Decimal` → string/number tại biên server trước khi đưa vào client component (Decimal không serialize sạch).
