...
Trước khi thêm 1 Link/router.push MỚI trỏ tới 1 route đã có `backHref`/`closeHref` khai sẵn, tự hỏi: (1) route đích đã có lối vào nào khác chưa (grep `ROUTES.<tên route>` toàn repo)? (2) `backHref`/`closeHref` tĩnh hiện tại có còn đúng với lối vào MỚI này không? Nếu (2) là "không" → dùng `withFrom`/`resolveBackHref` (`lib/routes.ts`), KHÔNG để `backHref` sai âm thầm (lỗi này không lộ ở typecheck/build, chỉ lộ khi bấm nút).

Cơ chế: entry point gắn `?from=<path hiện tại>` bằng `withFrom(href, fromPath)` — `fromPath` LUÔN là 1 giá trị `ROUTES.*` (path tĩnh hoặc kết quả hàm route động như `ROUTES.holdingDetail(id)`), không tự bịa string gợi nhớ. Page đích gọi `resolveBackHref(from, fallback)`: dùng THẲNG `from` làm backHref nếu là internal path an toàn, fallback về hành vi tĩnh cũ khi vắng/không hợp lệ (deep-link/bookmark vẫn đúng). "An toàn" được validate bằng cách để `URL` tự chuẩn hoá rồi so `origin` (`new URL(value, base).origin === base`), KHÔNG tự so tiền tố chuỗi tay (`startsWith("//")`...) — 1 lần làm vậy đã lọt biến thể `"/\evil.com"` (backslash bị trình duyệt coi tương đương `//` theo WHATWG URL spec), xem `process/decisions/architecture-and-code-quality.md` 2026-08-20. Không có whitelist tên cố định/`sourceMap` riêng từng route — nguồn có thể là path ĐỘNG (vd `/holdings/<id>`), page đích không cần verify ownership riêng vì mọi query đã tự filter theo `userId` (quy ước cốt lõi CLAUDE.md) — `backHref` chỉ là URL để `Link` trỏ tới, không phải cổng cấp quyền.

Ca thật: `/holdings/[id]/transactions/new`, `/holdings/[id]/price` (2 nguồn), `/holdings/new`, `/snapshots` (nguồn `holdingId` động) — cả 4 dùng CHUNG một cơ chế `from=<path>`, không còn tách riêng theo "nguồn tĩnh vs nguồn động" — xem `process/decisions/architecture-and-code-quality.md` 2026-08-20. `/settings` là ca khác hẳn: root tab không có màn cha, xử lý đúng là bỏ back arrow, không phải thêm `from`.
