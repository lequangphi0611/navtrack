...
- **Vị thế mở ban đầu** (khi tạo `Holding` lần đầu) được mô hình hóa như **một `Cashflow` kiểu BUY** đặt tại ngày mốc: `quantity` = số lượng đang giữ, `pricePerUnit` = giá vốn bình quân, `amount` = số âm tương ứng. Không cần model riêng — XIRR tính từ mốc này trở đi. **Không phải lúc nào cũng "không phí"** — chỉ đúng cho nhánh khai báo vị thế **đã có sẵn từ trước** (`intent: "HISTORICAL"`, không import lịch sử, `feeAmount = 0` vì phí quá khứ đã nằm sẵn trong giá vốn user tự nhập). Nhánh **vừa mua mới thật** (`intent: "NEW_PURCHASE"`, issue #142) dùng đúng công thức BUY chuẩn có phí (`TRANSACTION_FEE_BUY_<LOẠI>`, effective-dated), y hệt mọi giao dịch BUY khác — xem `docs/domain/02-transactions-and-cost-basis.md` mục "Vị thế mở ban đầu".
