...
- **Khi nhận cổ tức cổ phiếu**, giá vốn bình quân **bị pha loãng (dilute)** theo công thức đóng — `process/DECISION.md` 2026-08-13, chi tiết công thức + ví dụ ở `03-dividends.md` mục "Cách tính":
  ```
  avgCost mới = (SL trước × avgCost cũ) / (SL trước + SL cổ tức nhận thêm)
  ```
  Nhận CP thưởng không tốn thêm tiền, nên tổng vốn gốc giữ nguyên nhưng chia đều cho nhiều CP hơn → giá vốn trên mỗi CP giảm. Trước `process/DECISION.md` 2026-08-13, tài liệu này (và code) từng ghi "avgCost giữ nguyên khi nhận cổ tức cổ phiếu" — đó là **bug** (khiến lãi/lỗ đã thực hiện khi bán sau đó bị tính THẤP hơn thực tế), không phải quy tắc domain đúng.
...
- Giá vốn bình quân **dẫn xuất từ chuỗi `Cashflow` VÀ cổ tức cổ phiếu** (replay `derivePosition()`, `lib/cost-basis.ts` — hàm DUY NHẤT tính cả `quantity`/`avgCost`/`wentNegative`, replay CẢ `Cashflow` lẫn `Dividend{STOCK}`, issue #59, xem `01-assets-and-holdings.md` mục "Cách tính") — **nguồn sự thật là `Cashflow`** (+ `Dividend{STOCK}` cho cả phần số lượng LẪN phần pha loãng avgCost, xem trên). Để tránh replay toàn bộ lịch sử mỗi lần đọc màn Danh mục, giá trị này được **materialize** vào cột `Holding.avgCost` (kèm `Holding.quantity`) dưới dạng **cache dẫn xuất**, với bất biến: chỉ ghi lại trong **cùng transaction** với mọi thay đổi cashflow (không cộng/trừ tay) → cache luôn khớp nguồn, không tự lệch. Xem `docs/rules/data-prisma.md` (mục "Materialized cache…") và `process/DECISION.md` (2026-07-11) cho lý do đảo hướng "không lưu cứng" ban đầu. Giá vốn **tách khỏi XIRR** — XIRR dùng chuỗi dòng tiền thật theo ngày, không dùng giá vốn.

## Vị thế mở ban đầu
Khi tạo `Holding` lần đầu (`createHolding`), form phân biệt **2 kịch bản** qua `intent` (`HISTORICAL`/`NEW_PURCHASE`, issue #142) — cả hai đều tạo `Holding` + **một `Cashflow` BUY tại ngày mốc**, nhưng khác nhau ở cách tính `avgCost`/phí và cách xử lý trùng mã:

- **Khai báo vị thế đã có sẵn (`intent: "HISTORICAL"`)** — không phải giao dịch thật hôm nay, chỉ ghi lại một vị thế đang giữ từ trước (không import lịch sử đầy đủ): `quantity` = SL đang giữ, `pricePerUnit`/`avgCost` = giá vốn bình quân **user tự nhập** (đã gồm mọi phí quá khứ, app không tách lại được), `feeAmount = 0` trên `Cashflow` này (phí không nhập rời — đã nằm trong giá vốn user gõ), `amount` = số âm tương ứng. **Trùng mã** ở nhánh này **tự gộp im lặng** vào `Holding` đã có (find-or-create, xem mục "Gắn giao dịch vào Holding" trên) — coi như một lần khai báo bổ sung, không phải giao dịch mới cần phân biệt.
- **Vừa mua mới thật khi tạo Holding lần đầu (`intent: "NEW_PURCHASE"`)** — một giao dịch mua thật vừa khớp lệnh hôm nay, dùng **đúng công thức BUY chuẩn** ở mục "Cách tính" phía trên, không phải công thức riêng: `avgCost = (SL × giá + phí) / SL`, `feeAmount` **tự tính** từ `TRANSACTION_FEE_BUY_<LOẠI>` effective-dated theo ngày giao dịch — **giống hệt** cơ chế prefill của `TransactionForm` (mua thêm cho holding đã có), chỉ khác nơi hiển thị là form tạo Holding lần đầu. **Chặn ngày tương lai** (validate server-side): một giao dịch mua thật không thể xảy ra ở tương lai — khác nhánh `HISTORICAL` (vị thế đã có từ trước, ngày mốc có thể là ngày quá khứ bất kỳ) và khác cả `TransactionForm`/`addTransactionSchema` (mua thêm cho holding đã có, hiện **không** chặn ngày tương lai — khác biệt có chủ đích, xem `process/decisions/transactions-and-cost-basis.md` 2026-08-19). **Trùng mã** ở nhánh này **báo lỗi** thay vì gộp im lặng: một giao dịch mua thật ghi nhầm vào vị thế cũ sẽ trộn lẫn giá vốn hai lô mà user không hề chọn thế — server đọc số liệu `Holding` đã tồn tại (quantity/unit/avgCost, từ cache đã materialize sẵn) để hiển thị trong thông báo lỗi, hướng user sang ghi nhận ở màn giao dịch mua thêm thay vì tạo Holding mới.

XIRR và lãi/lỗ tính **từ mốc này trở đi** (không gồm quá khứ trước mốc), áp dụng cho cả hai nhánh. Vị thế mở ban đầu **không phải entity riêng** ở cả hai nhánh — dùng lại `Cashflow` kiểu BUY.
