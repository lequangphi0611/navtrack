...
## Cách tính
- **Số lượng hiện tại** = Σ(BUY.quantity) − Σ(SELL.quantity) + Σ(dividend STOCK.stockQuantity). Cache `Holding.quantity` phản ánh **cả BUY/SELL lẫn cổ tức cổ phiếu** — đường ghi cổ tức cổ phiếu (`features/dividends/actions.ts::recordDividend`, Phase 4/#52) **cộng thẳng `stockQuantity` vào cache hiện có** trong cùng transaction, không recompute lại toàn bộ (cổ tức chỉ CỘNG THÊM, không có nhánh "bán vượt" cần validate lại từ đầu). `avgCost` **bị pha loãng (dilute)** theo công thức đóng khi nhận cổ tức cổ phiếu — sửa lần 2, `process/DECISION.md` 2026-08-13 (trước đó tài liệu này ghi "avgCost giữ nguyên" — đó là **bug**, không phải quy tắc domain đúng: nhận CP thưởng không tốn thêm tiền nên phải làm giảm giá vốn bình quân trên mỗi CP) — xem `03-dividends.md`.
- **4 action mua/bán** (`features/holdings/actions.ts::createHolding/addTransaction/updateTransaction/deleteTransaction`) ghi cache bằng `derivePosition()` (`lib/cost-basis.ts`) — hàm DUY NHẤT xử lý CẢ `Cashflow` (BUY/SELL) LẪN cổ tức cổ phiếu, không có một hàm "gốc" chỉ-Cashflow nào tồn tại song song (issue #59, sửa lần đầu ở `process/DECISION.md` 2026-07-16 (4), dọn trùng lặp + đổi tên lại thành `derivePosition()` ở 2026-07-24 (4)): một cài đặt chỉ biết `Cashflow` từng gây (1) ghi đè mất phần `stockQuantity` đã cộng ở cache mỗi khi có giao dịch mua/bán sau đó, và (2) báo "bán vượt" SAI cho một lệnh bán mà SL bán nằm trong phần cổ tức cổ phiếu đã nhận (không phải mua). `derivePosition()` tính SL phát lại đúng thứ tự thời gian gồm cả `Dividend{STOCK}` qua `buildQuantityTimeline()` (`lib/position-trail.ts`, dùng chung cả `features/holdings/` lẫn `features/dividends/`); `avgCost` đổi bởi **CẢ BUY LẪN cổ tức cổ phiếu** (`process/DECISION.md` 2026-08-13), dùng SL thực (gồm cổ tức) làm cơ sở bình quân di động.
- `getHoldingDetail()` (`features/holdings/queries.ts`, tính "vị thế TẠI cutoff") cũng dùng `derivePosition()` — cùng lý do trên.
