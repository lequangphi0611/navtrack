...
## Entity / field
- `Dividend`: `holdingId`, `type` (`CASH`/`STOCK`/`BOND_COUPON`), `date`, `paymentDate?`, `grossAmount?`, `taxAmount?`, `netAmount?`, `stockQuantity?`, `parValueApplied?`, `couponRatePercentApplied?`, `note?`.
...
- Vì `Dividend` không lưu `percent`, màn lịch sử suy ngược:
  ...
  - `BOND_COUPON`: **lãi suất không suy ngược** — đọc thẳng `couponRatePercentApplied` đã đóng băng trên dòng đó, hiển thị dạng lãi suất năm (vd `9%/năm · kỳ 6 tháng`), không phải "% cổ tức". Đây là đáp án cho điểm mở (3) cũ của Phase 7: không cần công thức suy ngược cho lãi suất, vì thông số đã được lưu tại thời điểm ghi.
    - **Nhưng KỲ TRẢ LÃI thì có suy ngược** (chốt lúc implement #58): schema chỉ đóng băng `parValueApplied`/`couponRatePercentApplied`, **không** đóng băng `couponFrequencyMonths`. Đọc kỳ trả lãi từ `BondTerms` hiện tại sẽ vi phạm bất biến "sửa điều khoản không làm đổi **nhãn** của kỳ đã ghi" (nhãn gồm cả `kỳ 6 tháng`). Đảo ngược công thức `computeBondCoupon()` cho ra đúng kỳ đã dùng lúc ghi mà không cần thêm cột: `freq = grossAmount × 1200 / (parValueApplied × couponRatePercentApplied × SL_tại_ngày_ghi)` — `deriveCouponFrequencyMonths()` (`features/dividends/dividend-percent-label.ts`). Không ra số tháng nguyên (user đã sửa tay số tiền) hoặc SL = 0 → trả `null`, ẩn phần `· kỳ N tháng` thay vì bịa một kỳ không có thật.
