...
- Vì `Dividend` không lưu `percent`, màn lịch sử suy ngược:
  ...
  - `BOND_COUPON`: **không suy ngược** — đọc thẳng `couponRatePercentApplied` đã đóng băng trên dòng đó, hiển thị dạng lãi suất năm (vd `9%/năm · kỳ 6 tháng`), không phải "% cổ tức". Đây là đáp án cho điểm mở (3) cũ của Phase 7: không cần công thức suy ngược nào cả, vì thông số đã được lưu tại thời điểm ghi.
