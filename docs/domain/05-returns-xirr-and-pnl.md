...
- **`realizedPnl` track đúng số lượng thực nắm giữ kể cả khi có cổ tức cổ phiếu** (issue #83 code review #1): cổ tức cổ phiếu không tạo `Cashflow` (cộng thẳng vào `Holding.quantity`, xem `01-assets-and-holdings.md`), nên khi tính lãi/lỗ chốt phải phát lại CẢ `Cashflow` lẫn cổ tức cổ phiếu để biết đúng lúc một vị thế THỰC SỰ đóng hết (kể cả phần số lượng đến từ cổ tức) — nếu chỉ phát lại `Cashflow`, một lệnh bán hợp lệ (nhờ có thêm cổ phiếu từ cổ tức) có thể để lại phần dư âm/dương không được reset, làm sai `avgCost` cho các lô mua tiếp theo. **`avgCost` vẫn CHỈ derive từ `Cashflow` (BUY/SELL), KHÔNG đổi bởi cổ tức cổ phiếu** — giữ nguyên quy tắc cũ, cổ tức cổ phiếu chỉ ảnh hưởng tới việc "khi nào reset", không ảnh hưởng công thức bình quân di động (xem `lib/realized-pnl.ts`).
