...
- **XIRR toàn danh mục (làm rõ, quyết định 2026-07-21) — gộp MỘT chuỗi duy nhất, giải MỘT lần, KHÔNG cộng dồn/bình quân XIRR riêng của từng `Holding`:** vì XIRR không cộng tuyến tính (XIRR của hai vị thế riêng lẻ không thể cộng hay bình quân theo tỷ trọng để ra XIRR gộp đúng), XIRR toàn danh mục lấy `Cashflow` + `Dividend` của **MỌI `Holding` của user** (cả đã đóng lẫn đang mở) gộp chung thành một chuỗi dòng tiền duy nhất, cộng thêm **một** dòng tiền giả định dương = `NAV(danh mục)` hiện tại tại mốc chốt (đại diện phần đang mở — vị thế đã đóng không cần vì đã có dòng bán thật trong chuỗi), rồi giải XIRR một lần trên chuỗi gộp đó. Cách tính này đã đúng trong code hiện có (`lib/portfolio-valuation.ts` gộp XIRR cả danh mục, không giữ riêng theo `Holding`) — đoạn này chỉ nêu tường minh nguyên tắc vào domain doc.
...
- **`realizedPnl`/`avgCost` (write-path) track đúng số lượng thực nắm giữ kể cả khi có cổ tức cổ phiếu** (issue #83 code review #1, sửa lần 2 — `process/DECISION.md` sau `2026-07-24 (2)`): cổ tức cổ phiếu không tạo `Cashflow` (cộng thẳng vào `Holding.quantity`, xem `01-assets-and-holdings.md`), nên khi phát lại lịch sử để tính `avgCost`/lãi-lỗ chốt phải dùng **một bộ đếm `realQuantity` duy nhất** gồm CẢ `Cashflow` lẫn cổ tức cổ phiếu để biết đúng lúc một vị thế THỰC SỰ đóng hết (kể cả phần số lượng đến từ cổ tức). `avgCost` đổi ở sự kiện BUY (bình quân gia quyền), dùng `realQuantity` NGAY TRƯỚC sự kiện đó (không phải một bộ đếm chỉ-`Cashflow` riêng) làm cơ sở: `newAvgCost = (realQuantityTrước×avgCostCũ + tiềnMua) / (realQuantityTrước+SLMua)`. Khi vị thế đóng hết thật (`realQuantityTrước = 0`), số hạng `0×avgCostCũ = 0` **tự "quên" avgCost cũ — không cần bước reset tường minh nào nữa**, đúng cho CẢ ca đóng hết vị thế lẫn ca bán một phần (không đóng hết) rồi mua tiếp (thiết kế "2 bộ đếm song song + reset tường minh" trước đó chỉ đúng cho ca đóng hết, sai ở ca bán một phần). **Sửa lần 2 (`process/DECISION.md` 2026-08-13):** `avgCost` **CŨNG đổi bởi cổ tức cổ phiếu** (dilute theo công thức đóng, TRƯỚC khi cộng dồn `realQuantity`) — câu cũ ở đây ("avgCost CHỈ đổi bởi BUY, KHÔNG đổi bởi cổ tức cổ phiếu") là **bug**, không phải quy tắc domain đúng: bỏ qua dilute khiến `realizedGain` tính THẤP hơn thực tế khi bán sau khi đã nhận cổ tức cổ phiếu (xem `lib/cost-basis.ts::derivePosition`, `lib/realized-pnl.ts::computeRealizedGainForHolding`, `03-dividends.md` mục "Cách tính" cho công thức đóng đầy đủ).
...
- **"Dividend" trong chuỗi trên gồm `CASH` VÀ `BOND_COUPON`** (Phase 7) — `STOCK` không sinh tiền nên không có mặt. Tập này có **một nguồn sự thật** là hằng số `CASH_FLOW_DIVIDEND_TYPES` (`lib/enums.ts`), và **mọi** truy vấn gom cổ tức cho XIRR phải lọc qua nó, kể cả truy vấn nằm ngoài `features/*/repository.ts`. Sót một chỗ là trái tức biến mất khỏi đúng chỗ đó mà không test nào fail: bản đầu của #58 bỏ quên `getAllCashDividendsForXirr()` (`lib/portfolio-valuation.ts`), khiến trái tức vào XIRR của **từng vị thế** nhưng biến mất khỏi XIRR/`absolutePnl`/chi phí ăn mòn **cấp danh mục** — hai con số lệch nhau âm thầm, và một trái phiếu chính phủ giữ tới đáo hạn (toàn bộ lợi nhuận nằm ở coupon) tụt XIRR về gần 0. Sửa 2026-07-29, xem `process/DECISION.md`.
