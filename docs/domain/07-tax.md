...
- **Mức phí theo loại tài sản khác nhau (mới):** mỗi loại × mỗi chiều một key `TRANSACTION_FEE_<chiều>_<LOẠI>` (đặt `0` nếu chưa dùng kênh có phí, như `BOND`/`GOLD` hiện tại) — cùng nguyên tắc với `SALE_TAX_<LOẠI>` ở trên.
