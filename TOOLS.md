...
| Chạy E2E test (Playwright)                                                            | `pnpm e2e` — **chạy được**, không cần Docker: DB dùng Postgres native cài sẵn trong sandbox qua **setup script của environment** (cấu hình ngoài repo, ở phần cấu hình environment của Claude Code on the web — nội dung script ở README.md mục "Chạy e2e trên Claude Cloud", chạy 1 lần lúc provision, KHÔNG chạy tự động trong `pnpm e2e`). Nếu setup script chưa cấu hình/chưa chạy, `pnpm e2e` báo lỗi rõ ("Postgres chưa sẵn sàng...") thay vì tự cài hay báo pass giả — lúc đó mới thật sự phải skip. Xem `process/decisions/agent-workflow-and-tooling.md` mục 2026-08-13 | `pnpm e2e` — bắt buộc trước khi báo hoàn thành (xem `HARNESS.md`)           |
| Soi UI component qua browser (component cô lập ở `/preview`, **không** cần Docker/DB) | Playwright MCP qua `.mcp.json` → `scripts/playwright-mcp.mjs` (tự ép `--executable-path /opt/pw-browsers/chromium`). **Chạy được** — khác hẳn E2E ở trên: render component thuần, không cần Postgres                                                                                                                                                                                                                                                                                                                                                                             | Playwright MCP như cột Cloud (browser tự resolve, không ép executable-path) |
